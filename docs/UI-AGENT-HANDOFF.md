...
| **MacroHedgeStrategist** (Agent 1) | ✅ LIVE on Somnia 50312 at `0xf0570CcB1271FFaFf4caCA628F3632257f177b1D`; emits `StrategistDecided` — Phase 18 proved the two-leg `requestSchoolDecision` → `requestNotionalDecision` flow on-chain (two full mandates, `decisionState == (true, true)`, decision MOVES SHILLER `0x5` → POST_KEYNESIAN `0x6` on divergent input). Address/ABI/tx-hashes in `contracts/script/out/somnia-strategist-deployment.json` | Somnia testnet 50312 (LIVE) | prompt → hedge MANDATE (§4) + the school label |
...
**Agent-1 LIVE (DO subscribe to `StrategistDecided` on Somnia 50312).** The two-leg `MacroHedgeStrategist` is deployed LIVE at `0xf0570CcB1271FFaFf4caCA628F3632257f177b1D` (Somnia testnet 50312) and emits `StrategistDecided(decisionId, school, HedgeMandate)` on a real `requestSchoolDecision` → `requestNotionalDecision` flow (PROVEN on-chain, Phase 18: two full two-leg mandates landed, `decisionState == (true, true)`, and the decision MOVES — run-1 `SHILLER_MACRO_RISK` (`0x5`) vs run-2 `POST_KEYNESIAN` (`0x6`) on a divergent intent+consensus). Subscribe to it and decode the `HedgeMandate`; the live address, ABI inputs, the example `decisionId`, and the three real run-1 tx hashes (`schoolTx`/`notionalTx`/`strategistDecidedTx`) are in `contracts/script/out/somnia-strategist-deployment.json` (+ the ABI in `contracts/script/out/MacroHedgeStrategist.abi.json`). The v1 `0xfA428171E1F5B56f92C67C002De1d8e90B053EE1` stays reachable as the recorded-trace fallback. Somnia is NEVER forked — its consensus-validated inference + the keeper-proxy do not survive a fork; subscribe to the real testnet emission. The no-bridge honesty label above still holds (the `HedgeMandate` is carried Agent-1→Agent-2 by this app, not a cross-chain bridge).
