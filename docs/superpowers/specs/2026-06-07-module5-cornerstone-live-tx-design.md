...
**Status:** REVISED v4 — SCOPE CHANGE (user decision 2026-06-07): BOTH agents now run FULLY LIVE every iteration ("Option B"). Governing exec plan is `docs/cornerstone-live-twochain-PLAN.md` (T0–T8). This spec's Agent-2 mint, §0 honesty contract, §2 ABI/decoders, and the mint-side freshness gate REMAIN valid; the Agent-1 sections below (recorded-only) are SUPERSEDED by the v4 governance block. v2 passed 2-way review; v3 folded the handoff deltas (D1–D6); v4 records the live-Agent-1 pivot. GSD Phase 9 to be RE-PLANNED to Option B.
