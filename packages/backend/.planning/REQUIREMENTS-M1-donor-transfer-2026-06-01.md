...
**Defined:** 2026-05-25
**Core Value (from PROJECT.md):** M1 imputes a canonical parametric agentic cost function onto Somnia's `{json-fetch 0.03, llm-inference 0.07, llm-parse-website 0.10}` class structure by transfer from mature donor ecosystems — a forward/structural prior validated by donor-held-out calibration, NOT an estimate of Somnia's realized cost (unobservable until organic demand exists). Somnia supplies the settlement mechanics (trace-verified) and the posted-price anchor only. **[2026-05-31 re-scope — see `RESCOPE-SOMI-LEG-2026-05-31-v2.md` (gate-approved).]** *(Prior Core Value — superseded 2026-05-31, retained for audit: which caller-side inequalities in `somnia_cost_extraction.md §6` (rows #1, #2, #4, #5, #6, #7) bind empirically, where they are slack, and the realized `executionCost_i` distribution per agent class vs the docs-quoted `{0.03, 0.07, 0.10}` SOMI prices across observed impl versions.)*
...
- [~] **INDEX-01** (in progress — 03-01 Wave 0 decode surface + 03-02 Wave 0 pure-logic validation engine + 03-03 subgraph authoring [proxy-pinned manifest, flat Request schema with non-lossy CommitteeDepositFailed fold, AssemblyScript mappings, manifest-lint] done; 03-04 live probes remain) — Indexer deployed against the proxy address `0x5E5205CF…163E6`, **on the data source DATA-SOURCE-01 selects** (Ormi free tier if it cleared the sufficiency bar; the costed paid alternative otherwise). **Blocked by DATA-SOURCE-01 — not authored until the free-vs-paid verdict lands.** Owns subgraph authorship (`subgraphs/iagentrequester/`: schema.graphql, AssemblyScript mappings, networks.yaml for chain 5031) per KPD-08. Backfilled from contract deployment block (resolved in plan-phase from creator EOA `0x320362C7…fdE88936`) to present. Reconciliation against direct-RPC per KPD-04 (per-window parity, not random row sample); the parity mechanism is conditional on the DATA-SOURCE-01 capability-matrix `eth_getBlockReceipts` row. **[SUPERSEDED 2026-05-31 by RESCOPE-SOMI-LEG-2026-05-31-v2.md — the subgraph/eth_call/organic-Somnia approach is retired; replaced by donor-transfer estimation. Retained for audit history.]**

- [x] **SHARED-SCHEMA-01** — Joint-analysis schema, explicitly marked `v1-K_AI-anchored` with documented breakage budget when K_D ships. Two-table design: `schemas/abrigo_cost_panel_intersection_v1.md` (strict intersection columns) + `schemas/abrigo_cost_panel_k_ai_extensions_v1.md` (K_AI sidecar). Joint-analysis consumers in `abrigo-analytics` join on intersection PK. **[2026-05-31 re-scope (RESCOPE-SOMI-LEG-2026-05-31-v2.md): the K_AI cost term is now the imputed posterior-predictive `C_SOMI` distribution (donor-transferred, donor-held-out-calibrated), NOT a natively-observed `executionCost`.]**

### Donor-Transfer Estimation (M1 re-scope)

*Added 2026-05-31 per `RESCOPE-SOMI-LEG-2026-05-31-v2.md` (gate-approved, rev 3). M1 no longer measures Somnia's cost directly; it imputes a canonical agentic cost function from mature donor ecosystems and transfers it onto Somnia's class structure. These five requirements replace the retired subgraph/eth_call/organic-Somnia data-source path (DATA-SOURCE-01 / INDEX-01, now SUPERSEDED).*

- [ ] **DONOR-PANEL-01** — Build donor cost panels with provenance + POST-CENSOR surviving N (not raw counts): (A) commercial LLM per-token price matrix; (B) HAL/Cost-of-Pass per-task token-consumption panel (gated by DONOR-PANEL Wave-0 HAL feasibility); (C) on-chain realized-price panels — Virtuals ACP (Dune q7434268), OLAS Mech (q7623657/65), x402 (q7623533) — each reconciled against an independent total within a stated coverage tolerance; (D) RENDER aggregate-only organic prior. 6-key provenance header on every parquet.
- [ ] **SLOPE-EST-01** — Identify the resource slope `(p_in,p_out)` and consumption dispersion `σ(class)` OFF-CHAIN ONLY (LLM price matrix × HAL consumption). Gated by the Wave-0 HAL feasibility gate (strict per-class cost ordering json-fetch<llm-inference<llm-parse-website AND held-out class-assignment accuracy ≥ committed floor, frozen before decrypt); on FAIL the slope is declared non-identified and M1 is anchoring-only (no silent extrapolation).
- [ ] **TRANSFER-FIT-01** — Hierarchical partial-pooling (PyMC) fit transferring the canonical cost function onto Somnia as a zero-row held-out group with class fixed-effects CLAMPED to {0.03,0.07,0.10}. Selection: covariate-only ONLY for externally-labelled donors (x402/Artemis); MANDATORY two-part/Tobit censored likelihood for threshold-derived-label donors (ACP/OLAS) under the explicit conditional-ignorability premise; a frozen hash-logged pre-registration precedes any fit. Cost uses a per-donor redundancy_factor (Somnia φ=subSize=3 attestation), distinct from the reliability R_pass.
- [ ] **IDENT-AUDIT-01** — Identification audit: per-parameter posterior-contraction (`1−Var_post/Var_prior`) ≥ pre-registered τ certified by SBC (rank-uniformity) + prior-sensitivity on data-identified params; data-identified vs prior-only ledger mechanically enforced (κ,B0,rebate,burn are prior-only — never reported with a credible interval); slope-orthogonality gate (drop on-chain rows → structural-zero loading on slope on fail). Milestone success = donor-held-out calibration + LODO max-swing within threshold; if Somnia is outside the donor convex hull / prior-dominated, output labelled SCENARIO not estimate.
- [ ] **FX-STOCHASTICS-01** — SOMI/USD FX adapter (off-chain CoinGecko/Messari; no native oracle) + propagate FX as a random variable through the posterior-predictive (default: a dependence structure coupling depeg to load; independence only as justified fallback) → report SOMI-cost predictive distribution with P95/P99 tail quantiles. Feasibility floor governs quantitative-vs-qualitative.
...
- [ ] **BYTECODE-01** — Recover the rebate equation by tiered fallback:
  ...
  - **[RE-SCOPED 2026-05-31: now an OPTIONAL rebate-equation cross-check (trace is ground truth); not load-bearing.]**
...
|---|---|---|
| DATA-SOURCE-01 | Phase 1: Data-Sourcing Gate, Pre-flight Addendum & Schema Foundations | Superseded (2026-05-31, RESCOPE-SOMI-LEG-v2 — donor-transfer; prior status: Complete 01-01/01-03) |
...
| INDEX-01 | Phase 3: Subgraph Indexing (blocked by DATA-SOURCE-01) | Superseded (2026-05-31, RESCOPE-SOMI-LEG-v2 — donor-transfer; prior status: In Progress, 03-01/02/03 done) |
| DONOR-PANEL-01 | M1 re-scope (Waves 0–E) | Planned |
| SLOPE-EST-01 | M1 re-scope (Waves 0–E) | Planned |
| TRANSFER-FIT-01 | M1 re-scope (Waves 0–E) | Planned |
| IDENT-AUDIT-01 | M1 re-scope (Waves 0–E) | Planned |
| FX-STOCHASTICS-01 | M1 re-scope (Waves 0–E) | Planned |
