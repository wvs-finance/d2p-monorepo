...
| **AGENT-05** | unit | `get_iteration_state(app, slug)` returns a SINGLE-OBJECT result `{ status:'found'|'not_found', detail: IterationDetailOut|null }` (outputSchema is a ZodObject, never a ZodUnion — SDK normalizeObjectSchema is object-only). FOUND: on-site research-entry detail (slug/title/type/track/date/authors/summary + body + external_url notebook link + arxiv_id citable id; NO replication_hash/notebook_url — they do not exist in the data model). UNKNOWN: honest `status:'not_found', detail:null`. A REAL-SDK `tools/call` round-trip (found + not_found) in CI proves validateToolOutput passes | `lib/mcp-tools/get-iteration-state.ts` | ⬜ pending |
...
- [ ] **REAL-SDK output-schema test** (NEW-BLOCKER guard): `tests/api/mcp-real-sdk.test.ts` round-trips `get_iteration_state` through a real `McpServer.registerTool` + `Client.callTool` over InMemoryTransport (found + not_found) so a non-ZodObject outputSchema (which `normalizeObjectSchema` turns into `undefined` → TypeError in `validateToolOutput`) fails LOUDLY in CI. (The fake-server unit tests skip validateToolOutput, so this real round-trip is required.)
- [ ] **single-object outputSchema** (AGENT-05): `get_iteration_state` registers a single ZodObject `{ status, detail }` outputSchema, NOT a `z.union([...])`; both branches return matching structuredContent (the live-handshake checkpoint also calls it found+unknown)
...
- [x] Every AGENT-0X requirement has an automated verify or a recorded manual waiver
...
**Approval:** approved 2026-05-29 — per-task map complete, Wave 0 (incl. REAL-SDK output-schema guard) enumerated, all AGENT-0X verified-or-waived. `wave_0_complete: false` remains (the scaffolds turn green at execution); the validation DOC itself is signed-off/ready.
