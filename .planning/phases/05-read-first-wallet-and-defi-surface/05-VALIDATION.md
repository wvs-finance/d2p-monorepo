...
| recharts bundle isolation (WAIVER-05-05) | DEFI-04 | needs a build analysis | After `pnpm build` (NO `--analyze`/`@next/bundle-analyzer`): the chunk paths in `.next/diagnostics/route-bundle-stats.json` are content-hashed/opaque, so (1) identify the recharts-bearing chunk by body grep on a minification-surviving marker — `grep -rl "recharts-surface\|recharts-line\|recharts-wrapper" .next/static/chunks/` (recharts emits these DOM classNames at runtime) — then (2) assert that hashed filename is ABSENT from the `firstLoadChunkPaths` of every `(lab)`/`(apps)`/`(dashboard)` route and present only on the `(defi)` instrument route. Absence from non-`(defi)` routes is the binding FOUND-11 check. |
